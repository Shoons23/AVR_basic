...
void fnd_write_numbers_ultra(uint16_t num){
	...
	uint8_t value_u4 = num  % 10; // 1의 자리 
	uint8_t value_u6 = (num / 10) % 10; // 10의 자리
	uint8_t value_u8 = num / 100; // 100의 자리
...
void fnd_write_numbers_decimal(float num){    
	uint16_t value = (uint16_t)(num * 10);  // 25.5 -> 255로 변환
	uint8_t dot_off = 0x00;
	uint8_t dot_on = 0x01;
	
	uint8_t value_decimal = value % 10;      // 소수점 자리 (5)
	uint8_t value_ones = (value / 10) % 10;  // 일의 자리 (5)
	uint8_t value_tens = value / 100;        // 십의 자리 (2)
